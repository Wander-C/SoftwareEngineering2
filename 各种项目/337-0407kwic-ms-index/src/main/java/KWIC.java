  ...
  public void input(String file){
    try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        lineIndices.add(charsList.size());
        for (char c : line.toCharArray()) {
          charsList.add(c);
        }
      }
      chars_ = new char[charsList.size()];
      for (int i = 0; i < charsList.size(); i++) {
        chars_[i] = charsList.get(i);
      }
      line_index_ = lineIndices.stream().mapToInt(i -> i).toArray();
    } catch (IOException e) {
      e.printStackTrace();
      System.exit(1);
    }
...
//----------------------------------------------------------------------
  ...
  public void circularShift(){
    ...
    for (int lineIdx = 0; lineIdx < line_index_.length; lineIdx++) {
      int start = line_index_[lineIdx];
      int end = (lineIdx < line_index_.length - 1) ? line_index_[lineIdx + 1] : chars_.length;
      ...
      for (int wordStart : wordStarts) {
        shifts.add(new int[]{lineIdx, wordStart});
      }
    }
    circular_shifts_ = new int[shifts.size()][2];
    shifts.toArray(circular_shifts_);
  }
...
//----------------------------------------------------------------------
  ...
  public void alphabetizing(){
    alphabetized_ = Arrays.copyOf(circular_shifts_, circular_shifts_.length);
    Arrays.sort(alphabetized_, Comparator.comparing(this::getShiftedString));
  }
...
//----------------------------------------------------------------------
  ...
  public void output(){
    for (int[] shift : alphabetized_) {
      System.out.println(getShiftedString(shift));
    }
  }

  private String getShiftedString(int[] shift) {
    int lineIdx = shift[0];
    int wordStart = shift[1];
    int lineStart = line_index_[lineIdx];
    int lineEnd = (lineIdx < line_index_.length - 1) ? line_index_[lineIdx + 1] : chars_.length;
    List<Integer> wordStarts = new ArrayList<>();
    boolean inWord = false;
    ...
    int wordIndex = wordStarts.indexOf(wordStart);
    int numWords = wordStarts.size();
    StringBuilder sb = new StringBuilder();
    for (int i = 0; i < numWords; i++) {
      int currentIndex = (wordIndex + i) % numWords;
      int start = wordStarts.get(currentIndex);
      int end = (currentIndex < numWords - 1) ? wordStarts.get(currentIndex + 1) : lineEnd;
      sb.append(new String(chars_, start, end - start).trim()).append(" ");
    }

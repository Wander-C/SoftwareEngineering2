...
public void setup(LineStorage lines) {
  ...
  // 遍历原始输入的每一行
  for (int lineIdx = 0; lineIdx < lines.getLineCount(); lineIdx++) {
    String[] originalLine = lines.getLine(lineIdx);
    int wordCount = originalLine.length;
    ...
    for (int shift = 0; shift < wordCount; shift++) {
      String[] shiftedLine = new String[wordCount];

      // 构建移位后的行
      for (int i = 0; i < wordCount; i++) {
        int originalPos = (shift + i) % wordCount;
        shiftedLine[i] = originalLine[originalPos];
      }

      // 将移位结果存入存储
      shifts_.addLine(shiftedLine);
    }
  }
}

 ...
 */
  ...
  public void execute(String file){
    ...
    input.parse(file, ls);  // 假设Input类有parse方法存储数据到LineStorage

    // 2. 生成循环移位
    shifter.setup(ls);      // 将原始数据传入CircularShifter

    // 3. 字母表排序
    alphabetizer.alpha(shifter); // 假设Alphabetizer接收CircularShifter

    // 4. 结果输出
    output.print(alphabetizer); // 调用Output类的打印方法
  }
